...
func (s *service) Check(at accesstoken.AccessToken) (*accesstoken.AccessToken, error) {
	if err := accesstoken.ValidAccessToken(at.AccessToken); err != nil {
		logger.Log.Debug("Invalid access token")
	...
	if dbAccessToken.AccessToken != at.AccessToken {
		logger.Log.Debug("Access token is mismatch")
	...
	if dbAccessToken.IsExpired() {
		...
		if dbAccessToken.IsDelete() {
			...
			logger.Log.Debug("Access token is expired, and delete access token")
			return nil, ge.InvalidArgument("Access token is expired")
		}
		//更新(時間のみ)
		dbAccessToken.UpdateExpirationTime()
		if err := s.dbRepo.UpdateExpirationTime(*dbAccessToken); err != nil {
			return nil, err
		}
	}
